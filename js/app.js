...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  function renderHarvests(){const visible=harvestFilter==='all'?harvestRecords:harvestRecords.filter(task=>task.priority===harvestFilter);harvestList.innerHTML='';visible.forEach(task=>{const item=document.createElement('li'),label=task.priority==='medium'?'Moderate':task.priority[0].toUpperCase()+task.priority.slice(1),note=task.note?`<span class="harvest-note">${task.note}</span>`:'';item.innerHTML=`<span class="harvest-icon ${task.priority}">✓</span><span class="harvest-copy"><strong>${task.name}</strong>${note}</span><small>${label}</small>`;harvestList.append(item)});noHarvests.hidden=harvestRecords.length>0;document.querySelector('#all-count').textContent=harvestRecords.length;['urgent','medium','chill'].forEach(kind=>document.querySelector(`#${kind}-count`).textContent=harvestRecords.filter(task=>task.priority===kind).length)}
